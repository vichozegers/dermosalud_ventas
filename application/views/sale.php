<div class="container">
    ...
    <a class="btn btn-add btn-xs" data-toggle="modal" data-target="#ImportSales">Sube archivo CSV</a>
</div>
...
<script type="text/javascript">
    ...
    function upload_excel(){
        ...
        if (files.length > 0) {
            ...
            $.ajax({
                url: "<?php echo site_url('sales/upload_sales')?>",
                type: 'post',
                data: data,
                contentType: false,
                processData: false,
            ...
            alert('Debes agregar un archivo')
        }
...
<!-- /.Modal -->
<!-- /.Modal -->
<div class="modal fade" id="ImportSales" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title" id="myModalLabel"><?=label("AddSales");?></h4>
            </div>
            <?php
                $attributes = array('id' => 'addform');
                echo form_open_multipart('sales/importcsv', $attributes);
            ?>
            <div class="modal-body">
                <div class="form-group">
                    <label for="exampleInputFile"><?=label("UploadCSVfile");?></label>
                    <input type="file" name="userfile" id="ImageInput">
                    <p class="help-block"><a href="<?=site_url('files/sales_csv.csv');?>"><?=label('DownloadSample');?></a></p>
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal"><?=label("Close");?></button>
