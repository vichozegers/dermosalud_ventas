<div class="container">
   ...
   <div class="row rangeStat" style="margin-top:50px;">
      <h3 class="col-sm-12">Informe de Ventas</h3>
      <div class="col-md-5">
         <div class="form-group">
             <label for="customerSelect"><?=label('SelectStore');?></label>
               <select class="js-select-options form-control" id="StoresSelect">
      ...
      <div class="col-md-5">
            ...
            <div class="input-group margin-bottom-sm">
               <span class="input-group-addon RangePicker"><i class="fa fa-calendar fa-fw" aria-hidden="true"></i></span>
               <input class="form-control" id="SalesRange" type="text" name="daterangeV" />
            </div>
         </div>
      </div>
     
      <div class="col-md-2">
         <button class="cancelBtn btn btn-picker" type="button" onclick="verInformes()"><?='Generar Excel'?></button>
      </div>
   </div>
   <!-- ********************************************* warehouses report ***************************************************** -->
...
var store_id = $('#StoresSelect').find('option:selected').val();
      var Range = $('#SalesRange').val();
      var start = Range.slice(6,10)+'-'+Range.slice(0,2)+'-'+Range.slice(3,5);
      var end = Range.slice(19,23)+'-'+Range.slice(13,15)+'-'+Range.slice(16,18);
      
        $.ajax({
                url: 'informe/exportar2excel/'+start+'/'+end+'/'+store_id,
                dataType: 'text',
                type: 'post',
           ...
           //window.open('informe/exportar2excel/'+start+'/'+end+'/'+store_id);
    }
	</script>
